<template>
  <v-navigation-drawer app clipped width="340" v-model="drawer" class="scrollbar">
    ...
    <!--  -->
      ...
      <v-list-item>
        <v-list-item-content>
          <v-list-item-title class="title">Дополнительные сведения</v-list-item-title>
        </v-list-item-content>
      </v-list-item>
    </v-list>
    <v-divider></v-divider>
    ...
    <v-list nav>
      <div v-for="(e, i) in MenuLinksAddition" :key="i">
        <v-list-item link :to="{ name: e.path.name, params: { route: e.path.params } }" v-if="!e.hasSubmenu">
          <v-list-item-icon>
            <v-icon>mdi-{{e.icon}}</v-icon>
          </v-list-item-icon>
...
import linksSecond from './MenuLinksAddition'
export default {
  name: 'main-navigation-drawer',
  nameSecond: 'main-navigation-drawer',
  data () {
    return {
      drawer: false,
      menuLinks: links,
      MenuLinksAddition: linksSecond
    }
  },
  mounted () {
